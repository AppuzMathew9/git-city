...
function makeCrispMoonTexture(size = 512): THREE.CanvasTexture {
  ...
  const light = ctx.createRadialGradient(cx - r * 0.25, cy - r * 0.25, r * 0.15, cx, cy, r);
...
function makeSunsetDiscTexture(size = 512): THREE.CanvasTexture {
  ...
  const g = ctx.createRadialGradient(cx - r * 0.1, cy - r * 0.12, r * 0.08, cx, cy, r);
  g.addColorStop(0.0, "rgba(255,252,244,1)");
  g.addColorStop(0.3, "rgba(255,233,198,1)");
  g.addColorStop(0.62, "rgba(255,190,135,1)");
  g.addColorStop(0.9, "rgba(255,150,98,1)");
  g.addColorStop(1.0, "rgba(255,132,86,1)");
  ctx.fillStyle = g;
  ctx.fillRect(0, 0, size, size);
...
function makeSunsetHaloTexture(size = 512): THREE.CanvasTexture {
  ...
  g.addColorStop(0.0, "rgba(255,200,140,0.18)");
...
function makeSunsetCirrusDomeTexture(seed = 1, w = 1024, h = 512): THREE.CanvasTexture {
  ...
  for (let i = 0; i < streaks; i++) {
    ...
    const draw = (dx: number) => {
      ctx.beginPath();
      ctx.moveTo(x0 + dx, y);
      ctx.quadraticCurveTo(x0 + dx + len * 0.3, y - amp, x0 + dx + len * 0.65, y + amp * 0.6);
...
export default memo(function ThemeSkyFX({ themeIndex, theme }: Props) {
  ...
  const discMat = useMemo(() => {
    const m = new THREE.SpriteMaterial({
      ...
      color: discColor,
      blending:
        themeIndex === 1 || themeIndex === 3 ? THREE.AdditiveBlending : THREE.NormalBlending,
    });
    m.toneMapped = false;
  ...
  const starGeo = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const base =
        themeIndex === 0
          ? new THREE.Color(0.85, 0.92, 1.2) // cool blue-white (Midnight)
          : themeIndex === 2
            ? new THREE.Color(1.1, 0.8, 1.35) // lavender-white (Neon)
            : new THREE.Color(0.65, 1.1, 0.85); // cool green-white (Emerald)
  ...
  const dustGeo = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const pick = rng();
      const c = pick < 0.5 ? new THREE.Color(2.5, 0.4, 2.2) : new THREE.Color(0.4, 2.2, 2.5);
  ...
  const flyGeo = useMemo(() => {
    ...
    const palette = (theme.building?.windowLit ?? ["#39d353"]).map((h) => new THREE.Color(h));
  ...
  useEffect(() => {
    ...
    pulse.current = {
      ...
      dur: 0,
      active: false,
      nextIn: themeIndex === 2 ? randRange(rngRef.current, 4, 8) : randRange(rngRef.current, 8, 16),
    };
  ...
  const auroraMat0Ref = useRef(auroraRingMat0);
  auroraMat0Ref.current = auroraRingMat0;
  const auroraMat1Ref = useRef(auroraRingMat1);
  auroraMat1Ref.current = auroraRingMat1;
  const cirrusMatRef = useRef(sunsetCirrusMat);
  cirrusMatRef.current = sunsetCirrusMat;
  ...
  useFrame((_, delta) => {
    ...
    if (flyPointsRef.current) flyPointsRef.current.rotation.y += dt * 0.01;
    ...
    if (themeIndex === 3 && auroraRingMat0 && auroraRingMat1) {
      ...
      if (auroraRingMat0.map && auroraRingMat1.map) {
        auroraRingMat0.map.offset.x = (auroraRingMat0.map.offset.x + dt * 0.01) % 1;
    ...
    if (themeIndex === 0 && !prefersReducedMotion) {
      ...
      if (shower.current.active) {
        ...
        if (shower.current.nextIn <= 0 && shower.current.left > 0) {
          spawnStreak();
          shower.current.left--;
          shower.current.nextIn = randRange(rngRef.current, 0.22, 0.5);
        }
    ...
    if (pulse.current.nextIn <= 0 && !pulse.current.active) {
      ...
      const rng = rngRef.current;
      pulse.current.nextIn = themeIndex === 2 ? randRange(rng, 5, 12) : randRange(rng, 10, 22);
    }
    ...
    if (pulse.current.active) {
      ...
      mat.opacity = discOpacity + bump * (themeIndex === 2 ? 0.4 : 0.12);
      if (p01 >= 1) {
  ...
  useEffect(() => {
    return () => {
      ...
      // Aurora ring
      auroraGeo?.dispose();
      auroraRingMat0?.map?.dispose();
      auroraRingMat0?.dispose();
      auroraRingMat1?.map?.dispose();
      auroraRingMat1?.dispose();
      // Sunset scattering + halo + cirrus dome
      sunsetHazeGeo?.dispose();
      sunsetHazeMat?.map?.dispose();
      sunsetHazeMat?.dispose();
      sunsetHaloMat?.map?.dispose();
      sunsetHaloMat?.dispose();
      sunsetCirrusGeo?.dispose();
      sunsetCirrusMat?.map?.dispose();
      sunsetCirrusMat?.dispose();
      // Streaks
      streakGeo.dispose();
      streakMat.dispose();
    };
